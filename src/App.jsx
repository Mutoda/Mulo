...
/* ─────────────────────────────────────────────
   SCREEN 2B — PHONE NUMBER SELECTION
───────────────────────────────────────────── */
function PhoneSelect({ go }) {
  const [selected, setSelected] = useState(null);
  const [phase, setPhase] = useState("idle");

  // Mock phone numbers from CPB bureau — real data replaces this after CPB activation
  const bureauNumbers = [
    { masked: "+27 82* *** 1222", full: window._muloCellphone || "0829401222" },
    { masked: "+27 73* *** 4581", full: "0734574581" },
  ...
  return (
    <div className="screen fade-in">
      <div className="screen-header">
        ...
        <div className="screen-header-text">
          <div className="screen-header-title">Confirm your number</div>
      ...
      <div className="screen-scroll">
        <div style={{padding:"24px 20px 8px"}}>
          <div style={{fontSize:15,fontWeight:600,color:"#0A1628",marginBottom:6}}>
            We found these numbers linked to your ID
          </div>
          <div style={{fontSize:13,color:"#8FA3BE",marginBottom:24,lineHeight:1.6}}>
            Select the number you'd like to receive your WhatsApp verification code on.
          </div>

          {bureauNumbers.map((num, i) => (
            <div key={i}
              onClick={() => setSelected(num)}
              style={{
                display:"flex", alignItems:"center", gap:14,
                padding:"16px 18px", borderRadius:14, marginBottom:12, cursor:"pointer",
                border: selected?.full === num.full ? "1.5px solid #00B8A9" : "1.5px solid #E2E9F0",
                background: selected?.full === num.full ? "rgba(0,184,169,0.05)" : "#fff",
                transition:"all .2s"
              }}>
              <div style={{
                width:20, height:20, borderRadius:"50%", border:"2px solid",
                borderColor: selected?.full === num.full ? "#00B8A9" : "#C8D6E5",
                background: selected?.full === num.full ? "#00B8A9" : "transparent",
                display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0
              }}>
                {selected?.full === num.full && <div style={{width:8,height:8,borderRadius:"50%",background:"#fff"}}/>}
              </div>
              <div>
                <div style={{fontSize:16,fontWeight:700,color:"#0A1628",letterSpacing:1}}>{num.masked}</div>
                <div style={{fontSize:12,color:"#8FA3BE",marginTop:2}}>WhatsApp number</div>
              </div>
