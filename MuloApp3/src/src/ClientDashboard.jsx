 ...
 * Order: Telematics → Insure → Refinance
 */
...
const CLIENT = {
  ...
  insurance: {
    policies: [
      { product:'Car insurance',       insurer:'Naked Insurance', premium:689, status:'active', cashback:689, cashbackDate:'15 Jul 2026' },
      { product:'Buildings insurance', insurer:'Absa Insurance',  premium:521, status:'active', cashback:521, cashbackDate:'15 Jul 2026' },
    ],
    totalPremium:  1210,
    totalCashback: 1210,
...
export default function ClientDashboard() {
  const [expandTelem, setExpandTelem]   = useState(true)
  const [expandIns,   setExpandIns]     = useState(true)
  const [expandRef,   setExpandRef]     = useState(true)
    ...
    <SafeAreaView style={{ flex:1, backgroundColor:C.navy }} edges={['top']}>
      ...
      <View style={s.quickStats}>
        {[
          ...
          { label:'Active policies', value:String(insurance.policies.length),            color:C.amber },
          { label:'Cashback due',    value:'R'+insurance.totalCashback.toLocaleString(), color:C.green },
        ].map((stat,i) => (
          <View key={i} style={[s.quickStat, i===3 && { borderRightWidth:0 }]}>
      ...
      <ScrollView style={{ flex:1, backgroundColor:C.lightBg }}
        ...
        {/* ══ SECTION 1 — TELEMATICS ══ */}
        <TouchableOpacity onPress={() => setExpandTelem(p=>!p)} activeOpacity={0.8}>
          ...
          <View>
            ...
            <Card style={{ marginBottom:10 }}>
              <View style={{ flexDirection:'row', alignItems:'center', gap:16 }}>
                <View style={[s.scoreRing, { borderColor:C.teal }]}>
                  ...
                  <Text style={s.scoreSubLabel}>/ 100</Text>
                ...
                <View style={{ flex:1 }}>
                  ...
                  <View style={{ backgroundColor:C.green+'18', borderRadius:20, paddingHorizontal:10, paddingVertical:4, borderWidth:1, borderColor:C.green+'33', marginTop:8, alignSelf:'flex-start' }}>
                    <Text style={{ fontSize:11, color:C.green, fontWeight:'700' }}>
                      {'💚 Saving '+telematics.premiumSaving+'% on car insurance'}
                    </Text>
                  </View>
                  <Text style={{ fontSize:10, color:C.textMid, marginTop:4 }}>{'Tracker: '+telematics.trackerSource}</Text>
                </View>
              </View>
            </Card>
            ...
            <Card style={{ marginBottom:16, backgroundColor:C.teal+'12', borderWidth:1, borderColor:C.teal+'33' }}>
              ...
              <Text style={{ fontSize:13, color:C.textMid, lineHeight:20 }}>
                {'Your Gold Drive Score is saving you '}
                <Text style={{ color:C.green, fontWeight:'700' }}>{'R'+Math.round(689*0.09)+'/month'}</Text>
                {' on your Naked Insurance car premium. Keep driving safely to maintain your Gold band.'}
              </Text>
            </Card>
          </View>
        )}
        ...
        {/* ══ SECTION 2 — INSURE ══ */}
        <TouchableOpacity onPress={() => setExpandIns(p=>!p)} activeOpacity={0.8}>
            ...
            <Card style={{ marginBottom:10, backgroundColor:C.green+'18', borderWidth:1, borderColor:C.green+'33' }}>
              <View style={{ flexDirection:'row', alignItems:'center', justifyContent:'space-between' }}>
                <View>
                  ...
                  <Text style={{ fontSize:24, fontWeight:'800', color:C.green, marginBottom:2 }}>{'R'+insurance.totalCashback.toLocaleString()}</Text>
              ...
              <Card key={i} style={{ marginBottom:10 }}>
                ...
                <View style={{ flexDirection:'row', gap:10 }}>
                  <View style={{ flex:1, backgroundColor:C.lightBg, borderRadius:10, padding:10 }}>
                    ...
                    <Text style={{ fontSize:16, fontWeight:'700', color:C.textDark }}>{'R'+policy.premium+'/mo'}</Text>
                  </View>
                  <View style={{ flex:1, backgroundColor:C.green+'12', borderRadius:10, padding:10 }}>
                    <Text style={s.miniCardLabel}>Cashback</Text>
                    <Text style={{ fontSize:16, fontWeight:'700', color:C.green }}>{'R'+policy.cashback}</Text>
                  </View>
                </View>
        ...
        {/* ══ SECTION 3 — REFINANCE ══ */}
        <TouchableOpacity onPress={() => setExpandRef(p=>!p)} activeOpacity={0.8}>
          ...
          <View>
            ...
            <View style={{ flexDirection:'row', gap:10, marginBottom:10 }}>
              <Card style={{ flex:1 }}>
                ...
                <Text style={{ fontSize:20, fontWeight:'800', color:C.textDark, marginBottom:2 }}>{'R '+refinance.nextRepay.toLocaleString()}</Text>
                <Text style={s.miniCardSub}>{refinance.nextRepayDate}</Text>
              </Card>
              <Card style={{ flex:1 }}>
                <Text style={s.miniCardLabel}>Monthly saving</Text>
                <Text style={{ fontSize:20, fontWeight:'800', color:C.green, marginBottom:2 }}>{'R '+refinance.monthlySaving.toLocaleString()}</Text>
        ...
        <View style={{ alignItems:'center', paddingTop:8 }}>
          <Text style={{ fontSize:11, color:C.textLight }}>Muḽo Financial Technologies (Pty) Ltd · NCR Registered · POPIA Compliant</Text>
          <Text style={{ fontSize:11, color:C.textLight, marginTop:2 }}>Muḽo Financial Services (Pty) Ltd · FSP 49169</Text>
        </View>
...
const s = StyleSheet.create({
  ...
  quickStatVal:      { fontSize:13, fontWeight:'800', marginBottom:2 },
  ...
  scoreSubLabel:     { fontSize:9, color:C.textMid },
